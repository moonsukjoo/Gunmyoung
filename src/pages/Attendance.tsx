    ...
    return (
      <td {...rest} className={cn("p-1 relative align-top", className)}>
        <div className={cn(
          ...
          <span className={cn("text-[9px] font-black", isSun && !isToday ? "text-red-500" : isToday ? "text-primary-foreground" : "text-muted-foreground")}>
            {format(date, 'd')}
          </span>
          {att && att.clockIn ? (
  ...
  return (
    <div className="space-y-4 pb-24 px-2">
      <header className="py-6 space-y-1">
          <p className="text-[10px] sm:text-xs font-black text-muted-foreground/80 uppercase tracking-[0.2em]">근태 모니터링</p>
          <h2 className="text-3xl font-black tracking-tight text-foreground leading-tight">근태 관리</h2>
      ...
      {/* Clocking Unit */}
      <Card className="border-none shadow-2xl bg-card rounded-[3rem] overflow-hidden border border-border relative">
        ...
        <CardContent className="p-8 space-y-8 relative z-10">
           <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-2 px-3 py-1 bg-muted rounded-full border border-border">
                ...
                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">실시간 근무 모니터링</span>
              </div>
              <span className="text-6xl font-black text-foreground tabular-nums tracking-tighter drop-shadow-2xl">{format(now, 'HH:mm:ss')}</span>
           </div>
           ...
           <div className="grid grid-cols-2 gap-4">
              <button 
                ...
                className={cn(
                  "h-24 rounded-[2rem] flex flex-col items-center justify-center gap-2 transition-all active:scale-95 shadow-xl border-b-[6px]",
                  todayAttendance?.clockIn 
                    ? "bg-muted text-muted-foreground/30 border-border" 
                    : "bg-blue-600 text-white border-blue-700/50 shadow-blue-500/20"
                )}
              >
              ...
              <button 
                ...
                className={cn(
                  ...
                  (!todayAttendance?.clockIn || !!todayAttendance?.clockOut)
                    ? "bg-muted text-muted-foreground/30 border-border"
                    : "bg-rose-600 text-white border-rose-700/50 shadow-rose-500/20"
                )}
              >
           ...
           {todayAttendance?.clockIn && (
             <div className="bg-muted rounded-3xl p-4 flex justify-between items-center border border-border">
                ...
                {todayAttendance.clockIn && !todayAttendance.clockOut && (
                  <Badge className="bg-emerald-500/20 text-emerald-500 border-none px-3 py-1 font-black animate-pulse rounded-xl">
                    현재 근무 중
                  </Badge>
                )}
             </div>
      ...
      <div className="grid grid-cols-2 gap-3">
         <div className="bg-blue-500/5 p-6 rounded-[2rem] border border-blue-500/10 flex flex-col gap-1 items-start shadow-xl">
            <div className="w-8 h-8 bg-blue-500/20 rounded-xl flex items-center justify-center text-blue-500 mb-2">
              <Clock className="w-4 h-4" />
            </div>
            <span className="text-[10px] font-black text-blue-500/80 uppercase tracking-[0.1em]">이달의 누적 근무</span>
            <span className="text-3xl font-black text-foreground">{stats.total.toFixed(0)}<span className="text-sm font-bold text-muted-foreground/40 ml-1">H</span></span>
         </div>
         <div className="bg-amber-500/5 p-6 rounded-[2rem] border border-amber-500/10 flex flex-col gap-1 items-start shadow-xl">
            <div className="w-8 h-8 bg-amber-500/20 rounded-xl flex items-center justify-center text-amber-500 mb-2">
              <Activity className="w-4 h-4" />
            </div>
            <span className="text-[10px] font-black text-amber-600/80 uppercase tracking-[0.1em]">초과 근무 시간</span>
            <span className="text-3xl font-black text-amber-600">{stats.ot.toFixed(0)}<span className="text-sm font-bold text-amber-600/40 ml-1">H</span></span>
         </div>
      </div>
      ...
      <Card className="border-none shadow-none bg-card rounded-2xl overflow-hidden border border-border">
         <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-6">
               ...
               <span className="text-sm font-black text-foreground">최근 근무 트렌드</span>
            </div>
      ...
      <div className="space-y-3">
         <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-2">
               ...
               <span className="text-sm font-black text-foreground">상세 내역 (최근 5건)</span>
            </div>
            <Button 
               variant="ghost" 
               size="sm" 
         ...
         <div className="space-y-2">
            {[...attendanceData]
              ...
              .slice(0, 5)
              .map((att) => (
               <div key={att.id} className="bg-card p-5 rounded-2xl border border-border flex items-center justify-between">
                  <div className="flex flex-col">
                     <span className="text-xs font-black text-foreground">{format(parseISO(att.date), 'MM월 dd일 (EEEE)', { locale: ko })}</span>
